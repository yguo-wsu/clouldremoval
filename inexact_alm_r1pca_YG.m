function [A_hat E_hat sigularValue] = inexact_alm_r1pca_YG(D, lambda, tol, maxIter,verbose)
...
while ~converged
    ...
    if 0
        [~, ~, ~, A_hat,~] = rank1decomp(D - E_hat + (1/mu)*Y,2);
    else
    ...
    funval = sum(diag(S)) + lambda * sum(abs(E_hat(:)));
...
%% Rank 1 decomp
% Just get rank 1 decomposition of data matrix of size dxN assuming d>>N.
function [x, alpha, m, ReconstructedData,error] = rank1decomp(D,method)
if ~exist('method','var')
    method = 1;
end
[d,n] = size(D);
...
switch method
    case 0
        K = cov(DD);
        [V,l] = eig(K); l=l(end); x = DD*V(:,end)/sqrt(l);
        alpha = l*V(:,end);
        ReconstructedData = x*alpha' + repmat(m,1,n);
        error = D - ReconstructedData;
    case 1
        K = DD'*DD;
        [V,l] = eig(K); l=l(end); x = DD*V(:,end)/sqrt(l);
        alpha = V(:,end)*sqrt(l);
